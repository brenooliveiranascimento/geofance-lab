...
/**
 * Sequence content. Kept apart from the planner so copy can change without
 * touching timing logic — and so the planner's tests never depend on wording.
 *
 * Order in these arrays *is* the delivery order.
 */
...
/**
 * Four weeks of seven messages. Week and position are derived from the array
 * index by the planner, so adding a week is just appending seven entries.
 */
